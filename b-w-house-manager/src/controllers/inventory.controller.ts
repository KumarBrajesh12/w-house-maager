...
export const registerItem = async (c: Context) => {
    try {
        ...
        const item = await inventoryService.registerItem(customerId, name, description, category, declaredValue);
...
export const getCustomerInventory = async (c: Context) => {
    try {
        ...
        const inventory = await inventoryService.getCustomerInventory(customerId);
